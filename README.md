...
- **Najlepsze ułożenie na kierunki DX (dipol, delta pionowa, delta pozioma):** przycisk „policz dla wszystkich kierunków” liczy w NEC-2 warianty anteny dla ośmiu głównych kierunków DX (W1, W6, Karaiby, PY, ZS, VK6, VK2, JA) i pokazuje listę przycisków. Dla każdego kierunku strona najpierw sprawdza maszty po kolei (M1, M2, M3) i dla każdego podaje najlepsze ułożenie drutu z jego kątem ramion albo obrotem delty, a przy maszcie, z którego się nie da, pisze dlaczego (granica działki albo przeszkody). „Polecany” to najlepszy z masztów. Nowy maszt (siatka co 2,5 m, podany jako odległość i azymut od najbliższego masztu) pojawia się tylko wtedy, gdy żaden istniejący się nie mieści albo nowy daje ponad 0,3 dB więcej. Dipol nie wisi na maszcie z deltą, a delta na zajętym maszcie pokazuje, co trzeba z niego zdjąć. Dla dipola strona liczy też wszystkie układy rozpięte między masztami: zasilanie na jednym maszcie i końce przywiązane do dwóch pozostałych (kąt V wynika wtedy z położenia masztów, np. zasilanie na M1, końce do M2 i M3 daje V ok. 78°), oraz prosty dipol z zasilaniem w połowie przęsła między dwoma masztami. Maszt ma wysokość anteny na jego szczycie z zakładki Maszty, a środek i końce dipola wiszą 0,5 m niżej (maszt bez anteny: wysokość z zakładki Dipol). Każdy układ jest liczony w NEC-2 jako dipol rezonansowy: jeśli drut jest dłuższy niż odległość do masztu, nadmiar zwisa pionowo przy maszcie (koniec co najmniej 2 m nad ziemią, odsunięty od masztu na lince). Zgięcie końca kosztuje mało, bo końce dipola niosą mały prąd, a antena zostaje w rezonansie. Gdy drut rezonansowy nie mieści się nawet z opuszczonymi końcami, strona bierze najdłuższy drut, jaki się mieści (do masztu i w dół do 2 m), i zasila go przez eATU; podaje też, ilu metrów brakuje do rezonansu. Tak wydłużony drut wypada lepiej niż krótszy drut z linką (na 40 m między M1 i M2 średnio o ok. 3 dB w kierunkach DX). Każdy układ ma podane długości drutu, opuszczonych końców i linek, straty w zasilaniu i zysk w ośmiu kierunkach DX, a kliknięcie pokazuje pełną symulację tego układu (na planie liny są przerywane, a opuszczone końce opisane). Wysokość końców można też ustawić samemu: pole „Końce przy masztach na … m” pod przyciskiem liczy wszystkie układy z tą wysokością końców (puste pole: 0,5 m pod anteną na maszcie), a po wybraniu układu pola „Wysokość końców przy masztach” pozwalają ustawić każdy koniec osobno. Każda zmiana od razu sprawdza w NEC-2, czy drut rezonansowy (z nadmiarem opuszczonym przy maszcie) nadal się mieści; jeśli nie, strona przechodzi na najdłuższy drut, jaki się mieści, z eATU. Końce niżej niż środek dają odwrócone V między masztami. Nie da się kliknąć tylko układu z zasilaniem na maszcie z deltą. Jeśli drut przechodzi przez korony drzew albo za nisko nad zabudowaniami, układ ma ostrzeżenie. Najlepszy układ bez ostrzeżeń pojawia się też w każdym kierunku obok M1, M2 i M3. Każdy wiersz podaje układ drutu, średni zysk przy 5–15° w tym kierunku (z terenem, budynkami, drzewami i stratami w kablu albo eATU) i straty w zasilaniu. Kliknięcie ustawia antenę w tym układzie i wybiera ten azymut. Dipol: kąt między ramionami od 180° (prosty) do 30° (wąskie V) co 5°, obrót co 1°. Delta pionowa: dolny drut 1–5 m nad ziemią, trzy punkty zasilania i linia dolnego drutu co 5°. Delta pozioma: punkt zasilania w każdym z trzech narożników albo na środku każdego boku. Drut musi się zmieścić w działce (0,5 m od granicy) i ominąć przeszkody na działce: nad altaną, szopą czy domkiem musi przejść co najmniej 1 m wyżej niż dach (i 1 m obok), a nad koroną drzewa co najmniej 0,5 m wyżej. Punkt zawieszenia nie może stać w budynku ani w koronie drzewa. Wyniki liczone są bez pozostałych anten na działce; po zmianie pasma, wysokości, gruntu albo kabla lista znika i trzeba ją policzyć jeszcze raz. Dipol liczy się ok. 15 s, delta ok. 7 s.
...
**Szerokie ekrany (np. 1920 × 768):** od 1400 px szerokości strona zajmuje całą szerokość okna. Tytuł i zakładki są w jednym wierszu, wskaźniki (KPI) w osobnej kolumnie po prawej, a mapa nieba, charakterystyki i plan dobierają rozmiar do wysokości okna. Panel z parametrami po lewej przewija się niezależnie od reszty strony, więc na niskim ekranie każdy suwak jest pod ręką obok wykresów.
